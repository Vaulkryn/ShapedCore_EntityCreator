...
function countShapes(node, shapeCounts) {
        ...
        commands.forEach(command => {
            ...
            switch (type) {
                ...
                case 'L':
                    ...
                    break;
                case 'C':
                    coordinates.push({ x: nums[0], y: nums[1] }, { x: nums[2], y: nums[3] }, { x: nums[4], y: nums[5] });
                    break;
                case 'Z':
                    break;
            }
...
function convertToGlobal(x_group, y_group, theta_group, x_local, y_local) {
    ...
    selection.forEach(node => {
        if (node.type === 'GROUP') {
            ...
            groupNode.children.forEach((shape, index) => {
                ...
                entityConfig += `        origin: { x: ${globalCoords.x}, y: ${globalCoords.y} },\n`;
                if (theta_shape == 0) {
                    entityConfig += '';
                }
                else {
                    entityConfig += `        rotation: ${theta_shape},\n`;
                }
                ...
                allCoords.forEach((coord, index) => {
                    if (index === allCoords.length - 1) {
                        entityData += `            { "x": ${coord.x}, "y": ${coord.y} }\n`;
                    }
                    else {
                        entityData += `            { "x": ${coord.x}, "y": ${coord.y} },\n`;
                    }
            ...
            entityData += `    }\n`;
            entityData += `}\n`;
            const totalShapes = shapeCounts.rectangles +
                shapeCounts.ellipses +
                shapeCounts.polygons +
                shapeCounts.stars +
                shapeCounts.lines +
